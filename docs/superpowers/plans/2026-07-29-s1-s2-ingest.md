...
`page.rotation_matrix` maps unrotated text space into displayed space, which is the space `get_pixmap()` renders. Applying it makes both agree. On an unrotated page it is the identity matrix, so it is always safe to apply.
...
def test_first_word_lands_at_known_position(born_digital_pdf):
    """Text starts at x=72pt on a 612pt page: 72/612 = 0.1176."""
    doc = fitz.open(born_digital_pdf)
    boxes = extract_boxes(doc[0])
    doc.close()
    ...
    assert abs(first.x0 - 72.0 / 612.0) < 1e-6
...
def test_rotated_page_boxes_match_rendered_space(rotated_pdf, born_digital_pdf):
    ...
    assert abs(rot_first.x0 - 688.41 / 792.0) < 1e-3
...
def test_normalized_box_maps_back_to_pixels(born_digital_pdf, tmp_path):
    ...
    # x=72pt at 150dpi is 150 px
    assert abs(px0 - 150.0) < 1.0
```
