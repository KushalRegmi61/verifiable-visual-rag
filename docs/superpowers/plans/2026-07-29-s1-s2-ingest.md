...
| `src/visual_verify/derive.py` | Line, block, and span unions over word boxes |
...
S3 gets a stable corpus of page images with known geometry to embed, and the `Sink` protocol to extend for vector upserts. S4 gets `extract_boxes` output as its candidate set and `derive.span_box` as the text-path implementation. S7 gets the word granularity its auto-derived gold boxes require.
